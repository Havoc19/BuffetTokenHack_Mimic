...
contract Ownable is Context {
  ...
  function blockWallet(address Wallet) external onlyOwner {
    ...
    function blockListWallets(address[] calldata addresses) external onlyOwner {
      for (uint256 i; i < addresses.length; ++i) {
        _isBlacklisted[addresses[i]] = true;
      }
    }
  function unblockWallet(address account) external onlyOwner {
...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  constructor() public { 
    ...
    _symbol = "BUFFETT";
    _decimals = 18;
    _totalSupply = 110000000000  * 10 ** 18 ; 
  ...
  function allowance(address owner, address spender) external view returns (uint256) {
    require(!_isBlacklisted[spender], "To/from address is blacklisted!");
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    require(!_isBlacklisted[spender] , "This address is blacklisted");
